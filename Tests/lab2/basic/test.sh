...
echo -e $YELLOW"Sample Test Set For Lab-2 Group-2.2"$RESET
...
# function noerrorTest() # input file, expect file
# {
#     INPUT=$1
#     EXPECT=$2
#     RES=$($PARSER $INPUT)
...
ERROR_TESTS=$(find inputs -name *.cmm | sort)
# HAS_ERROR_TESTS=$(find inputs/has_error -name *.cmm | sort)
# NO_ERROR_TESTS=$(find inputs/noerror -name *.cmm | sort)
...
# echo -e $YELLOW"--- No Error Testcases ---"$RESET

# for t in $NO_ERROR_TESTS
# do
#     e=${t##*/}
#     e="expects/"${e%.*}".exp"
#     # echo $t $e
